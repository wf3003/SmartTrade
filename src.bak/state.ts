...
export function applyOptRules(
  ...
  if (side === "long") {
    ...
    // #4 做多侧对等保护: RSI>70做多×0.5
    const rsiMidLong = interceptParamsCache.get("rsi_mid_long") ?? 70;
    if (rsi_1h > 0 && rsi_1h > rsiMidLong && rsi_1h < rsiLong) {
      score = Math.round(score * 0.5);
      logs.push(`[#4]RSI${rsi_1h.toFixed(0)}>${rsiMidLong} 做多 ×0.5`);
    }
  }
